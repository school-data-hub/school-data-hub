...
class SchoolSemester(db.Model):
    ...
    start = db.relationship(
        "Schoolday", foreign_keys=[start_date], backref="start_semester", uselist=False
    )
    end = db.relationship(
        "Schoolday", foreign_keys=[end_date], backref="end_semester", uselist=False
    )
...
class Schoolday(db.Model):
    ...
    missed_classes = db.relationship(
        "MissedClass", back_populates="missed_day", cascade="all, delete-orphan"
    )
    schoolday_events = db.relationship(
        "SchooldayEvent",
        back_populates="schoolday_event_day",
        cascade="all, delete-orphan",
    )
...
class MissedClass(db.Model):
    ...
    unexcused = db.Column(db.Boolean)
...
class SchooldayEvent(db.Model):
    ...
    # - RELATIONSHIP TO PUPIL MANY-TO-ONE
    schoolday_event_pupil_id = db.Column(
        "schoolday_event_pupil", db.Integer, db.ForeignKey("pupil.internal_id")
    )
    schoolday_event_pupil = db.relationship(
        "Pupil", back_populates="pupil_schoolday_events"
    )

    # - RELATIONSHIP TO SCHOOLDAY MANY-TO-ONE
    schoolday_event_day_id = db.Column(
        "schoolday_event_day", db.Date, db.ForeignKey("schoolday.schoolday")
    )
    schoolday_event_day = db.relationship(
        "Schoolday", back_populates="schoolday_events"
    )

    def __init__(
        self,
        schoolday_event_id,
        schoolday_event_pupil_id,
        schoolday_event_day_id,
        schoolday_event_type,
        schoolday_event_reason,
        created_by,
        processed,
        processed_by,
        processed_at,
