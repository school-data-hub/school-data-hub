...
class SupportCategoryStatus(db.Model):
    ...
    support_category_id = db.Column('support_category_id', db.Integer,
...
class SupportGoal(db.Model):
    ...
    support_category_id = db.Column('support_category_id', db.Integer,
                                 db.ForeignKey('support_category.id'))
    support_category = db.relationship('SupportCategory', back_populates='category_goals')

    #- RELATIONSHIP TO CHECKS ONE-TO-MANY
    goal_checks = db.relationship('SupportGoalCheck', back_populates='goal',
                                 cascade="all, delete-orphan")

    def __init__(self, pupil_id, support_category_id, goal_id, created_by, created_at, achieved,
...
class SupportCategory(db.Model):
    ...
    support_category_statuses = db.relationship('SupportCategoryStatus',
                                       back_populates='support_category')
...
class SupportGoalCheck(db.Model):
    ...
    goal_id = db.Column(db.String(50), db.ForeignKey('support_goal.goal_id'))
    goal = db.relationship('SupportGoal', back_populates='goal_checks')

    def __init__(self, goal_id, check_id,created_by, created_at, achieved, comment):
        self.goal_id = goal_id
