...
def get_authorization_by_id(authorization_id: str) -> Optional[Authorization]:
    return db.session.query(Authorization).filter_by(authorization_id=authorization_id).scalar()

def get_authorization_by_name(authorization_name: str) -> Optional[Authorization]:
    return db.session.query(Authorization).filter_by(authorization_name=authorization_name).scalar()
...
def get_missed_class_by_pupil_and_schoolday(pupil_id: int, schoolday_id: int) -> Optional[MissedClass]:
    return db.session.query(MissedClass).filter(
...
def get_schoolday_event_by_id(schoolday_event_id: str) -> Optional[SchooldayEvent]:
    return db.session.query(SchooldayEvent).filter(SchooldayEvent.schoolday_event_id == schoolday_event_id).first()
