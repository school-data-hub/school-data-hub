...
def upload_pupils_txt(current_user, files_data):
    ...
    pupils_to_delete = db.session.query(Pupil).filter(~Pupil.internal_id.in_(internal_ids)).all()
    ...
    if request.method == 'POST':
        ...
        for row in csv_reader:
                ...
                if first_row:
                    first_row = False
                    continue  # Skip it
                if db.session.query(exists().where(SupportCategory.category_id == row[0])).scalar() == False:
                    if row[1] == '':
                        this_parent_category = None
                    else:
                        this_parent_category = row[1]
                    category = SupportCategory(category_id=row[0],	parent_category=this_parent_category,	category_name=row[2])
