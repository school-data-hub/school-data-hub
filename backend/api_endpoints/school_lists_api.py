...
school_list_api = APIBlueprint(
    "school_list_api", __name__, url_prefix="/api/school_lists"
)
...
@school_list_api.doc(
    security="ApiKeyAuth",
    tags=["School Lists"],
    summary="Post a school list including ALL pupils in the database",
)
@token_required
def add_list_all(current_user, json_data):
    ...
    new_list = SchoolList(
        list_id, list_name, list_description, created_by, visibility, authorized_users
    )
...
@school_list_api.doc(
    security="ApiKeyAuth",
    tags=["School Lists"],
    summary="Post a school list including pupils from an array",
)
@token_required
...
@school_list_api.output(school_list_schema)
@school_list_api.doc(
    security="ApiKeyAuth", tags=["School Lists"], summary="Patch a school list"
)
@token_required
...
@school_list_api.doc(
    security="ApiKeyAuth",
    tags=["School Lists"],
    summary="Get all school lists with nested elements",
)
@token_required
...
@school_list_api.doc(
    security="ApiKeyAuth",
    tags=["School Lists"],
    summary="Get all school lists without nested elements",
)
@token_required
...
@school_list_api.doc(
    security="ApiKeyAuth",
    tags=["School Lists"],
    summary="Add pupil(s) to a school list",
)
@token_required
def add_pupil_to_list(current_user, list_id, json_data):
    ...
    for item in internal_id_list:
        ...
        if modified_pupil != None:
            if (
                PupilList.query.filter_by(
                    origin_list=list_id, listed_pupil_id=item
                ).first()
                == None
            ):
                origin_list = list_id
...
@school_list_api.output(school_lists_schema)
@school_list_api.doc(
    security="ApiKeyAuth", tags=["School Lists"], summary="Delete a school list"
)
@token_required
