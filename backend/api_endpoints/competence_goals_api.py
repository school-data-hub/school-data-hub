...
competence_goal_api = APIBlueprint(
    "competence_goal_api", __name__, url_prefix="/api/competence_goals"
)
...
@competence_goal_api.doc(
    security="ApiKeyAuth",
    tags=["Competence Goals"],
    summary="Post a goal associated with a competence",
)
@token_required
def post_competence_goal(current_user, internal_id, json_data):
    ...
    new_goal = CompetenceGoal(
        competence_goal_id=competence_goal_id,
...
@competence_goal_api.doc(
    security="ApiKeyAuth",
    tags=["Competence Goals"],
    summary="Patch a goal associated with a competence",
)
@token_required
...
@competence_goal_api.doc(
    security="ApiKeyAuth",
    tags=["Competence Goals"],
    summary="Delete a goal associated with a competence",
)
@token_required
def delete_competence_goal(current_user, goal_id):
    ...
    db.session.commit()
    return jsonify({"message": "Kompetenzziel gelöscht!"}), 200
