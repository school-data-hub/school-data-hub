...
@pupil_list_api.doc(
    security="ApiKeyAuth", tags=["Pupil Lists"], summary="Patch a pupil list entry"
)
@token_required
def patch_pupil_list(current_user, pupil_id, list_id, json_data):
    school_List = SchoolList.query.filter_by(list_id=list_id).first()
    pupil_list = PupilList.query.filter_by(
        origin_list=list_id, listed_pupil_id=pupil_id
    ).first()
...
@pupil_list_api.doc(
    security="ApiKeyAuth",
    tags=["Pupil Lists"],
    summary="Delete pupil entries from list",
)
@token_required
