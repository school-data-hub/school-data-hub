...
from helpers.db_helpers import get_missed_class_by_pupil_and_schoolday, get_pupil_by_id, get_schoolday_by_day
...
def add_missed_class(current_user, json_data):
    ...
    existing_missed_class = get_missed_class_by_pupil_and_schoolday(this_missed_pupil_id, this_missed_day_id) 
...
def add_missed_class_list(current_user, json_data):
    ...
    for entry in missed_class_list:
        ...
        missed_schoolday = db.session.query(Schoolday).filter(Schoolday.schoolday == missed_day).first()
        
        if missed_schoolday == None:
            abort(404, 'Dieser Schultag existiert nicht!')
        
        missed_class = get_missed_class_by_pupil_and_schoolday(missed_pupil_id, missed_schoolday.schoolday) 
        if missed_class != None:
            ...
            modified_by = None
            comment = None
            new_missed_class = MissedClass(missed_pupil_id, missed_schoolday, missed_type, excused,
                                        contacted, returned, written_excuse, minutes_late, returned_at,
                                        created_by, modified_by, comment)
            db.session.add(new_missed_class)
...
def delete_missed_class_with_date(current_user, pupil_id, date):
    ...
    missed_class = db.session.query(MissedClass).filter(MissedClass.missed_day_id == missed_day_id, MissedClass.missed_pupil_id == pupil_id ).first() 
    if missed_class == None:
