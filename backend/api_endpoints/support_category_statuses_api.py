...
from models.support_category import SupportCategory, SupportCategoryStatus
from schemas.log_entry_schemas import ApiFileSchema
from schemas.pupil_schemas import *
from schemas.support_schemas import *

support_category_status_api = APIBlueprint(
    "support_category_status_api", __name__, url_prefix="/api/support_category/statuses"
)
...
@support_category_status_api.doc(
    security="ApiKeyAuth",
    tags=["Support Category Statuses"],
    summary="Post a status for a given catagory from a given pupil",
)
@token_required
def add_support_category_status(current_user, internal_id, category_id, json_data):
    ...
    new_category_status = SupportCategoryStatus(
        pupil_id=pupil_id,
        support_category_id=goal_category_id,
        status_id=status_id,
        state=state,
...
@support_category_status_api.doc(
    security="ApiKeyAuth",
    tags=["Support Category Statuses"],
    summary="Patch a status for a given catagory from a given pupil",
)
@token_required
...
@support_category_status_api.doc(
    security="ApiKeyAuth",
    tags=["Support Category Statuses"],
    summary="PATCH-POST a file to document a given pupil category status",
)
@token_required
...
@support_category_status_api.doc(
    security="ApiKeyAuth",
    tags=["Support Category Statuses"],
    summary="Get file of a given pupil category status",
)
@token_required
...
@support_category_status_api.doc(
    security="ApiKeyAuth",
    tags=["Support Category Statuses"],
    summary="Delete a status for a given catagory from a given pupil",
)
@token_required
