...
from app import db
from helpers.db_helpers import get_pupil_by_id, get_schoolday_by_day, get_schoolday_event_by_id
from helpers.log_entries import create_log_entry
from models.log_entry import LogEntry
from models.schoolday import SchooldayEvent, Schoolday
...
schoolday_event_api = APIBlueprint('schoolday_events_api', __name__, url_prefix='/api/schoolday_events')

#- POST SCHOOLDAY EVENT  
##################
@schoolday_event_api.route('/new', methods=['POST'])
@schoolday_event_api.input(schoolday_event_in_schema, example={
...
@schoolday_event_api.doc(security='ApiKeyAuth', tags=['Schoolday Events'], summary='Post an schoolday event')
@token_required
def add_schoolday_event(current_user, json_data):
    ...
    new_schoolday_event = SchooldayEvent( schoolday_event_id= schoolday_event_id,
    ...
    db.session.commit()
    pupil = Pupil.query.filter_by(internal_id = schoolday_event_pupil_id).first()
    return pupil
...
@schoolday_event_api.doc(security='ApiKeyAuth', tags=['Schoolday Events'], summary='Get all schoolday events')
@token_required
...
@schoolday_event_api.doc(security='ApiKeyAuth', tags=['Schoolday Events'], summary='Get schoolday event by id')
@token_required
...
@schoolday_event_api.output(pupil_schema)
@schoolday_event_api.doc(security='ApiKeyAuth',tags=['Schoolday Events'], summary='Patch an schoolday event')
@token_required
...
@schoolday_event_api.output(pupil_schema)
@schoolday_event_api.doc(security='ApiKeyAuth',tags=['Schoolday Events'], summary='PATCH-POST a file to document a given pupil schoolday event')
@token_required
def upload_schoolday_event_file(current_user, schoolday_event_id, files_data):
    schoolday_event = get_schoolday_event_by_id(schoolday_event_id)
    if schoolday_event is None:
        return jsonify( {"message": "A schoolday event with this date and this student does not exist!"}), 404
...
@schoolday_event_api.input(ApiFileSchema, location='files')
@schoolday_event_api.output(pupil_schema)
@schoolday_event_api.doc(security='ApiKeyAuth',tags=['Schoolday Events'], summary='PATCH-POST a file to document a given pupil processed schoolday event')
@token_required
...
@schoolday_event_api.doc(security='ApiKeyAuth', tags=['Schoolday Events'], summary='Get file of a given schoolday event')
@token_required
def download_schoolday_event_file(current_user,schoolday_event_id):
    schoolday_event = get_schoolday_event_by_id(schoolday_event_id)
    if schoolday_event == None:
        abort(404, message="A schoolday event with this date and this pupil does not exist!")        
...
@schoolday_event_api.output(FileSchema, content_type='image/jpeg')
@schoolday_event_api.doc(security='ApiKeyAuth', tags=['Schoolday Events'], summary='Get file of a given processed schoolday event')
@token_required
def download_schoolday_event_processed_file(current_user,schoolday_event_id):
    schoolday_event = get_schoolday_event_by_id(schoolday_event_id)
    if schoolday_event == None:
        abort(404, message="An schoolday event with this date and this pupil does not exist!")        
...
@schoolday_event_api.output(pupil_schema)
@schoolday_event_api.doc(security='ApiKeyAuth', tags=['Schoolday Events'], summary='Delete schoolday event file of a given schoolday event')
@token_required
def delete_schoolday_event_file(current_user, schoolday_event_id):
    ...
    schoolday_event = get_schoolday_event_by_id(schoolday_event_id)
    if schoolday_event is None:
        return jsonify( {"message": "An schoolday event with this date and this student does not exist!"}), 404
    if len(str(schoolday_event.file_url)) < 5:
        abort(404, message='This schoolday event has no file!')       
    if len(str(schoolday_event.file_url)) > 4:
        os.remove(str(schoolday_event.file_url))
...
@schoolday_event_api.output(pupil_schema)
@schoolday_event_api.doc(security='ApiKeyAuth', tags=['Schoolday Events'], summary='Delete schoolday event processed file of a given processed schoolday event')
@token_required
...
@schoolday_event_api.output(pupil_schema)
@schoolday_event_api.doc(security='ApiKeyAuth', tags=['Schoolday Events'], summary='Delete an schoolday event and eventual files')
@token_required
